...
public class HomeController {
	...
	public ModelAndView LicenseUpload(HttpSession session,@ModelAttribute MemberVO memberVO, HttpServletResponse response) throws IOException {
		...
		if (!file.isEmpty()) {
			String FileName = file.getOriginalFilename();
			file.transferTo(new File("C:\\Users\\user\\Documents\\workspace-sts-3.9.5.RELEASE\\WithProject_0.0.5\\src\\main\\webapp\\resources\\"+FileName));
		}
		memberVO.setCertificate(file.getOriginalFilename());
	...
	//핸드폰 인증
		...
		public ModelAndView ChangePsPro(HttpSession session, HttpServletRequest request, @ModelAttribute MemberVO memberVO) throws IOException {
			System.out.println("비밀번호띠"+memberVO.getPassword());
			System.out.println("아디띠"+memberVO.getId());
			mav = new ModelAndView();
	...
	public void jusoPopup2(HttpServletResponse response,@RequestParam("roadAddrPart1") String roadAddrPart1) throws IOException {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		System.out.println(roadAddrPart1);
		out.println("<script>");
		out.println("opener.jusoCallBack('"+roadAddrPart1+"');");
	...
	/*//주소 2번째 테스트
	@RequestMapping(value = "jusoLet", method = RequestMethod.GET)
	public String jusoLet() {
		return "http://www.juso.go.kr/addrlink/addrLinkUrl.do";
	}
	*/
}
